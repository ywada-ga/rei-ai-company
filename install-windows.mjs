...
const environment=['REI_DATA_DIR','REI_BACKUP_DIR','REI_CONNECTOR_CONFIG','REI_PORT','REI_JOB_TIMEOUT_SECONDS'].filter(key=>process.env[key]).map(key=>`set ${safe(`${key}=${process.env[key]}`)}\r\n`).join('');
