...
const beforeRace=readdirSync(path.join(base,'backups')).sort();
const changingBackup=createBackup(source,path.join(base,'backups'));
queueMicrotask(()=>writeFileSync(path.join(source,'pending-results.json'),'[{"taskId":"changed-during-backup"}]'));
await assert.rejects(changingBackup,/バックアップ中に設定または送信待ち結果が変わりました/);
assert.deepEqual(readdirSync(path.join(base,'backups')).sort(),beforeRace);
writeFileSync(path.join(source,'pending-results.json'),'[{"taskId":"pending-report"}]');
