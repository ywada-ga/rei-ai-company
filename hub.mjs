...
function readyConnector() {
  ...
  const capabilities=devices.map(device=>JSON.parse(device.capabilities||'[]'));
  return capabilities.some(items=>items.includes('planning'))&&capabilities.some(items=>items.includes('execution'));
}
...
async function api(req,res,route) {
  ...
  if(route.startsWith('tasks/detail/')&&req.method==='GET') {
    const id=route.slice('tasks/detail/'.length);
    ...
    const eventRows=all(db,'SELECT id,actor,type,detail,created_at,task_id FROM events WHERE task_id=? OR task_id IN (SELECT id FROM tasks WHERE parent_id=?) ORDER BY created_at DESC,id DESC LIMIT 301',id,id);
  ...
  if(route==='users/role'&&req.method==='POST') {if(user.role!=='owner')return error(res,403,'所有者だけが権限を変更できます');const data=await body(req),id=String(data.userId||''),role=String(data.role||'');if(!['admin','requester','viewer'].includes(role))return error(res,400,'役割が正しくありません');const target=one(db,'SELECT id,username,role FROM users WHERE id=?',id);if(!target||target.role==='owner')return error(res,404,'変更できる利用者が見つかりません');transaction(db,()=>{run(db,'UPDATE users SET role=? WHERE id=?',role,id);event(db,null,user.username,'user_role',`${target.username}: ${role}`);});return send(res,200,{ok:true});}
  ...
  if(route==='chatwork/status'&&req.method==='GET')return send(res,200,chatworkStatus(db,root));
