...
REI本体もログイン時に自動起動させる場合は、初回登録後に `npm start` を `Ctrl+C` で止め、同じフォルダで `npm run install:hub` を実行します。macOS・Windows・Linuxで共通のコマンドです。既存のREI用自動起動設定があるPCではその設定を更新するため、導入前に [更新と復旧の手順](UPDATING.md) を確認してください。
