...
接続に困ったときは参加PCのREIフォルダで `npm run doctor` を実行してください。Node.js、OpenClaw、接続設定、中心PCへの到達、端末トークン、REIの版、送信待ち結果をまとめて確認します。診断結果には端末トークンを表示しません。エラーが出ても設定や仕事の記録は変更しません。送信待ち結果がある場合は、ファイルを消さずHubの仕事と照合してください。
