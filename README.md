...
初回起動時はブラウザで「REIの初期登録」が自動で開きます。開かない場合はターミナルに表示されたURLを開いてください。所有者の名前と14文字以上のパスワードを設定し、続けてログインします。画面右上の **端末・設定 → このPCの接続コードを作る** を押し、表示された1行をREIのフォルダで実行して接続コードを入力します。既存のOpenClawとは別にREI専用エージェントが作られ、Connectorが自動起動に登録されます。REIが既に起動中なら `npm run launch` はその画面を開きます。

REI本体もログイン時に自動起動させる場合は、初回登録後に `npm run launch` を `Ctrl+C` で止め、同じフォルダで `npm run install:hub` を実行します。macOS・Windows・Linuxで共通のコマンドです。既存のREI用自動起動設定があるPCではその設定を更新するため、導入前に [更新と復旧の手順](UPDATING.md) を確認してください。
...
接続に困ったときは参加PCのREIフォルダで `npm run doctor` を実行してください。Node.js、OpenClaw、工程の実行期限、モデルの認証設定、接続設定、中心PCへの到達、端末トークン、REIの版、担当AIの直接送信・管理操作の制限、送信待ち結果をまとめて確認します。モデルの確認は設定の検査であり、外部AIへの実通信は行いません。診断結果には端末トークンを表示しません。エラーが出ても設定や仕事の記録は変更しません。送信待ち結果がある場合は、ファイルを消さずHubの仕事と照合してください。

中心PCの状態は `npm run doctor:hub` で確認できます。初回登録、データベース、REI本体の応答、最新バックアップを検査し、7日以上バックアップしていなければ知らせます。1時間以上残っている未完成のバックアップも知らせます。実行中の工程があれば更新前の注意も表示します。診断では仕事や認証情報を変更しません。
...
OpenClawへ渡した1工程の実行期限は既定で30分です。実行中もConnectorは心拍と取得期限の更新を続けます。特殊な長時間作業では、Connectorを起動する環境で `REI_JOB_TIMEOUT_SECONDS` を60〜7200秒に設定できます。期限内に終わらない工程は成功扱いにせず、結果を確認してから再依頼してください。

**稼働報告 → 報告をコピー** で、今日の仕事、PC・人の稼働、以前から残る要確認を日本語の文章としてコピーできます。Chatworkの接続前でも、必要な相手へ手動で共有できます。
...
Hubと異なる版を報告したPCには新しい仕事を渡しません。更新前に得た仕事の結果は引き続き返送できるので、送信待ち記録を消さずに更新してください。
...
所有者は **端末・設定 → MCP連携** から、利用するPCと連携先を検索し、チェックして一括登録できます。国内向けのfreee、マネーフォワード クラウド会計・事業者情報、Misoca、kintone公式ドキュメント検索、SUZURIに加え、Meeting.ai、Notion、Linear、Jira・Confluence、Google Workspaceのカレンダー・メール・連絡先・横断検索など、Context7、Supabase、Sentry、Cloudflare、Upstash、Exa、Firecrawl、Stripeを選べます。画面には認証方式、利用条件、公式手順を表示します。対応範囲と調査結果は [MCP_JAPAN_CATALOG.md](MCP_JAPAN_CATALOG.md) と [MCP_TREND_CATALOG.md](MCP_TREND_CATALOG.md) に記録しています。REI ConnectorがそのPCのOpenClawへ `rei_` で始まる専用設定を反映します。既存のOpenClaw MCP設定には触れません。解除も対象PCが次に接続した時に反映されます。対象PCが停止中なら、画面では反映待ちになります。
...
ルームIDはChatworkの[公式ヘルプ](https://help.chatwork.com/hc/ja/articles/360000142942-%E3%83%AB%E3%83%BC%E3%83%A0ID%E3%82%92%E7%A2%BA%E8%AA%8D%E3%81%99%E3%82%8B)、APIトークンは[公式の開始手順](https://developer.chatwork.com/docs/getting-started)で確認できます。組織アカウントはAPI利用に管理者の承認が必要な場合があります。トークンはREIの設定画面へ直接入力し、チャットでは共有しないでください。
接続時にはChatworkの[自分のアカウントID](https://developer.chatwork.com/reference/get-me)も確認します。REI自身の投稿と投稿者を確認できないメッセージは回答として扱いません。投稿IDを確認できない送信は二重投稿せず「要確認」にします。暗号鍵が欠けたり壊れたりした場合は設定画面へ「要確認」を出し、新しい鍵を自動作成しません。バックアップから鍵を復元するか、所有者が接続を設定し直してください。
...
通信断の後に元の端末から結果が届いた場合は、工程の詳細にその報告を表示します。報告だけでは実施済みと断定せず、所有者か管理者が確認結果を記録します。確認を先に終えた後で端末の報告が届いた場合は、確認内容を残したまま仕事を再び「要確認」にし、両方を見て確認し直せます。

ミッション一覧は最新100件から表示し、古い仕事は **過去の仕事をさらに表示** で順に読み込めます。仕事の詳細では省略していない結果と工程履歴を確認できます。履歴は新しい30件から表示し、**古い履歴をさらに表示** で過去の記録を順に読み込めます。
...
- 所有者のログイン情報は初回登録時に決めます。`owner-credentials.txt` を作成した場合は公開リポジトリに含めず、所有者だけが読める場所に保管してください。ログイン後に設定画面でパスワードを変更できます。
- Connectorは接続先がHTTPS、またはSSH転送したlocalhostの場合にだけ起動します。
- MCP連携を追加した場合、対象PCのOpenClaw設定に `rei_` で始まる専用項目を追加します。それ以外の既存MCP設定は変更しません。かんたん端末追加では、個人用と別のワークスペース・会話履歴を持つ `rei` エージェントを作ります。既存の `main` をREI用に変更する選択肢もあります。明示的なツール設定がないREI用エージェントには、MCPとファイル・実行操作を使えるOpenClawの `coding` プロファイルを設定します。どちらもREI経由以外のメッセージ送信とGateway管理のツールを無効にします。`coding` でもOS上のファイルを操作できるため、クライアント配布時は端末ごとにOpenClawのツール権限と作業場所を確認してください。
- REI用作業場所や指示ファイルがシンボリックリンクなど通常のフォルダ・ファイルでない場合、エージェント設定を止めます。作業場所にある既存ファイルを確認してから再実行してください。
...
所有者は **端末・設定 → バックアップを作成** から、REIの稼働中にSQLiteの整合した写しを作れます。保存先は既定では中心PCの `data/backups/` です。仕事・利用者・端末・Chatwork暗号鍵・このPCのConnector設定と、送信待ちの結果を含みます。MCP設定と送信待ち結果の保存途中ファイルが残っている場合も保全します。対象ファイルがシンボリックリンクなど通常のファイルではない場合は、欠けたバックアップを作らずエラーにします。保存途中のバックアップは検証が終わるまで完成済み一覧へ表示されません。バックアップには認証情報が入るため、フォルダの権限は所有者だけに制限します。`owner-credentials.txt` は含みません。

外付けドライブなどへ自動保存する場合は、**先にドライブ上へバックアップ用フォルダを作成**し、Hubを起動する前に `REI_BACKUP_DIR` にその**絶対パス**を指定します。Macの例は `REI_BACKUP_DIR=/Volumes/PrivateDrive/rei-backups npm run install:hub`、Windows PowerShellでは `$env:REI_BACKUP_DIR='D:\rei-backups'; npm run install:hub` です。自動起動設定にも保存先が引き継がれ、画面からの手動作成と日次の自動作成が同じ保存先を使います。ターミナルから `npm run doctor:hub` や `node backup.mjs create` を実行する際も、同じ `REI_BACKUP_DIR` を指定してください。既存のバックアップは移動・削除されません。ドライブやフォルダが使えないときは別の場所へ黙って保存せず、Hubのログと診断に失敗を表示します。秘密情報を含むため、所有者だけがアクセスできる保存先を選んでください。

Hubは所有者登録後、起動から約1分後と以後1時間ごとに最新バックアップを検証し、24時間以上経っていれば新しく作成します。バックアップ中に設定や送信待ち結果が変わった場合は、不揃いな写しを残さず次回に再試行します。既存のバックアップは自動削除しないため、保存先の空き容量を定期的に確認してください。自動作成に失敗した場合はHubのログに理由を残し、手動作成も使えます。
...
復元は既存データを上書きせず、必ず新しいフォルダに展開します。検証ではファイルのSHA-256とSQLiteの整合性を確認します。復元先への展開は、別フォルダでコピーと整合性検査を済ませてから確定します。復元後はHubとConnectorを停止し、両方で `REI_DATA_DIR` を復元先に指定して起動してください。自動起動を使う場合は、復元先を指定した状態で各OSのインストーラーを再実行すると、次回のログイン後も同じ保存先を使います。端末トークンなどの秘密情報を含むため、バックアップを公開リポジトリに追加しないでください。

OpenClawエージェントを新しく準備する場合は、`REI_DATA_DIR` を指定してから `npm run agent:setup` を実行すると、その保存先に作業場所と変更前のOpenClaw設定の退避が作られます。既存のREI用作業場所は再設定しても維持します。このバックアップにはOpenClawの作業場所と設定は含まれません。別のPCへ移すときは、各PCのOpenClaw設定とREI用作業場所を別途保管・復旧してください。
