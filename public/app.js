...
async function loadNetworkStatus() {
  ...
  try {
    ...
    if(epoch!==state.authEpoch)return;
    if(status.state!=='connected'&&$('pairing-url').value.startsWith('https://'))$('pairing-url').value='';
    if(status.state==='missing')networkPanel.innerHTML='<strong>① このPCにTailscaleを入れる</strong><p>PC間を安全につなぐアプリです。Macでは公式のStandalone版を入れ、Macの管理者パスワードとVPNの許可を入力してください。設定後、追加するPCも同じTailscaleネットワークに参加させます。</p><a href="https://tailscale.com/download" target="_blank" rel="noopener noreferrer">公式サイトから入手 ↗</a> <a href="https://tailscale.com/docs/install/mac" target="_blank" rel="noopener noreferrer">Macの手順 ↗</a><button id="network-refresh" class="outline-button" type="button">接続を再確認</button>';
    ...
    else if(status.state==='ready')networkPanel.innerHTML=state.data?.user?.role==='owner'?'<strong>② このPCの接続を有効にする</strong><p>Tailscale内だけでREIを開けるようにします。</p><button id="network-enable" class="outline-button" type="button">安全な接続を有効にする</button>':'<strong>② このPCの接続は所有者が有効にします</strong><p>所有者に「安全な接続を有効にする」操作を依頼してください。</p>';
    else if(status.state==='connected'){networkPanel.innerHTML=`<strong>✓ このPCの接続準備ができました</strong><p>接続URL: <code>${escapeHtml(status.url)}</code></p><p>追加するPCにもTailscaleを入れ、同じアカウントでログインしてください。</p>`;$('pairing-url').value=status.url;}
    ...
    else if(status.state==='conflict'){networkPanel.innerHTML='<strong>Tailscaleの入口は別のサービスに使用中です</strong><p>既存のServe設定を確認してください。REIはその設定を自動で上書きしません。</p><button id="network-refresh" class="outline-button" type="button">接続を再確認</button>';$('pairing-url').value='';}
