...
if(process.platform==='linux') {
  ...
  for(const mode of ['hub','connector']) {
    const result=spawnSync(process.execPath,['install-linux.mjs',mode],{cwd:root,encoding:'utf8',env:{...process.env,REI_SYSTEMD_DIR:units,REI_NO_START:'1',REI_DATA_DIR:'/tmp/rei-restored-data',REI_BACKUP_DIR:'/tmp/rei-separate-backups',REI_PORT:'4188',REI_JOB_TIMEOUT_SECONDS:'3600'}});
    ...
    assert.match(unit,/REI_BACKUP_DIR=\/tmp\/rei-separate-backups/);
