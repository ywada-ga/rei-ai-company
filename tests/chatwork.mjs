import { mkdtempSync, lstatSync, renameSync, symlinkSync, unlinkSync } from 'node:fs';
...
assert.equal((await pollChatwork(db,dir)).received,0);
const keyFile=path.join(dir,'chatwork.key'),savedKey=path.join(dir,'chatwork.key.saved');
assert.equal(chatworkStatus(db,dir).needsAttention,false);
renameSync(keyFile,savedKey);
await assert.rejects(()=>pollChatwork(db,dir),/暗号鍵がありません/);
assert.equal(lstatSync(keyFile,{throwIfNoEntry:false}),undefined);
assert.equal(chatworkStatus(db,dir).needsAttention,true);
symlinkSync(savedKey,keyFile);
await assert.rejects(()=>pollChatwork(db,dir),/通常のファイルではありません/);
unlinkSync(keyFile);
renameSync(savedKey,keyFile);
assert.equal(chatworkStatus(db,dir).needsAttention,false);
