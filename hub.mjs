...
import { departments, event, createTask, cancellable, cancelTask, claim, sweep, finishJob, finishRoot, report } from './workflow.mjs';
...
function taskJson(t) {return {id:t.id,parentId:t.parent_id,kind:t.kind,text:t.text,department:t.department,projectId:t.project_id||null,status:t.status,assignedDeviceId:t.device_id,result:t.result,error:t.error,createdAt:new Date(t.created_at).toISOString(),startedAt:t.started_at?new Date(t.started_at).toISOString():null,finishedAt:t.finished_at?new Date(t.finished_at).toISOString():null};}
...
async function api(req,res,route) {
  ...
  if(route.startsWith('connector/')) {
    ...
    if(route==='connector/claim'&&req.method==='POST') {sweep(db);const job=claim(db,device);if(job?.project_id)job.project=one(db,'SELECT name,objective FROM projects WHERE id=?',job.project_id)||null;const devices=all(db,'SELECT id,label,capabilities FROM devices WHERE revoked=0').map(d=>({...d,capabilities:JSON.parse(d.capabilities)}));return send(res,200,{job,devices});}
