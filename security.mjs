...
export function connectorDevice(db,req) { const raw=String(req.headers.authorization||'').match(/^Bearer (.+)$/)?.[1]; return raw?one(db,'SELECT id,label,planner,capabilities FROM devices WHERE token_hash=? AND revoked=0',hash(raw)):null; }
