...
const environment=['REI_DATA_DIR','REI_CONNECTOR_CONFIG','REI_PORT','REI_JOB_TIMEOUT_SECONDS'].filter(key=>process.env[key]).map(key=>`Environment=${quote(`${key}=${process.env[key]}`)}`).join('\n');
