...
try {
  ...
  const failedPair=await fetch(`${base}/api?route=connector%2Fpair`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:firstPairing.code})});
  ...
  pairDb.close();
  const badPairCheck=await fetch(`${base}/api?route=connector%2Fpair%2Fcheck`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:'INVALIDCODE12345'})});
  assert.equal(badPairCheck.status,403);
  const firstPaired=await api('connector/pair',{code:firstPairing.code});
  const usedPairCheck=await fetch(`${base}/api?route=connector%2Fpair%2Fcheck`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:firstPairing.code})});
  ...
  const revokedStatus=await fetch(`${base}/api?route=connector%2Fstatus`,{headers:{authorization:`Bearer ${firstPaired.token}`}});
  ...
  const failedEnroll=await fetch(`${base}/api?route=devices%2Fenroll`,{method:'POST',headers:{'content-type':'application/json',cookie},body:JSON.stringify({label:'failed-enroll',isPlanner:true})});
  ...
  writeFileSync(path.join(data,'connector.json'),JSON.stringify({hub:base,token:'invalid',agent:'rei'}));
  assert.equal((await api('devices')).localConnector.status,'needs_attention');
  writeFileSync(path.join(data,'connector.json'),JSON.stringify({hub:base,token:auth,agent:'rei'}));
  ...
  const failedMcpAdd=await fetch(`${base}/api?route=mcp%2Fadd`,{method:'POST',headers:{'content-type':'application/json',cookie},body:JSON.stringify({label:'Failed Calendar',url:'https://example.com/failed-mcp',auth:'oauth',deviceId:enrolled.device.id})});
  ...
  const failedMcpRemove=await fetch(`${base}/api?route=mcp%2Fremove`,{method:'POST',headers:{'content-type':'application/json',cookie},body:JSON.stringify({name:mcp.name})});
  ...
  const replay=await fetch(`${base}/api?route=connector%2Fpair`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:pairing.code})});
  ...
  const conflictingResult=await api('connector/result',{taskId:execute.job.id,leaseId:execute.job.lease_id,success:true,result:'違う結果'},auth);
  ...
  for(let index=0;index<450;index++)insertEvent.run(`00000000-0000-4000-8001-${String(index).padStart(12,'0')}`,created.task.id,'test','history',`履歴 ${index}`,1700000000000+index);
  eventDb.close();
  const recentHistory=await api(`tasks/detail/${created.task.id}`);
  const recentEvents=recentHistory.events;
  assert.equal(recentEvents.length,300);
  assert.equal(recentHistory.hasOlderEvents,true);
  assert.ok(recentEvents.some(item=>item.detail==='履歴 449'));
  ...
  const olderEvents=await api(`tasks/events/${created.task.id}`,{beforeTime:Date.parse(oldest.createdAt),beforeId:oldest.id});
  assert.equal(olderEvents.events.length,100);
  assert.equal(olderEvents.hasMore,true);
  assert.ok(olderEvents.events.every(item=>item.createdAt<=oldest.createdAt));
  const earliest=olderEvents.events[0];
  const oldestEvents=await api(`tasks/events/${created.task.id}`,{beforeTime:Date.parse(earliest.createdAt),beforeId:earliest.id});
  assert.equal(oldestEvents.hasMore,false);
  assert.ok(oldestEvents.events.some(item=>item.detail==='履歴 0'));
  const allEvents=[...recentEvents,...olderEvents.events,...oldestEvents.events];
  assert.equal(new Set(allEvents.map(item=>item.id)).size,allEvents.length);
  ...
  const cancelledAgain=await fetch(`${base}/api?route=tasks%2Fcancel`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:stopped.id})});
  ...
  assert.ok(!(await api('bootstrap')).humanPending.includes(human.id));
  const repliedAgain=await fetch(`${base}/api?route=human%2Frespond`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:humanStep.id,answer:'重複'})});
  assert.equal(repliedAgain.status,409);
  const auditDb=new DatabaseSync(path.join(data,'rei.sqlite'));
  auditDb.exec("CREATE TRIGGER fail_project_status_event BEFORE INSERT ON events WHEN NEW.type='project_status' BEGIN SELECT RAISE(ABORT,'project status event failure'); END;");
  const failedProjectStatus=await fetch(`${base}/api?route=projects%2Fstatus`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({projectId:project.id,status:'paused'})});
  assert.equal(failedProjectStatus.status,500);
  assert.equal(auditDb.prepare('SELECT status FROM projects WHERE id=?').get(project.id).status,'active');
  auditDb.exec('DROP TRIGGER fail_project_status_event');
  await api('projects/status',{projectId:project.id,status:'paused'});
  const paused=await fetch(`${base}/api?route=command`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({text:'保留中の依頼',projectId:project.id})});
  ...
  const forbidden=await fetch(`${base}/api?route=projects%2Fcreate`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({name:'不可',objective:'不可'})});
  ...
  auditDb.exec("CREATE TRIGGER fail_user_role_event BEFORE INSERT ON events WHEN NEW.type='user_role' BEGIN SELECT RAISE(ABORT,'user role event failure'); END;");
  const failedRole=await fetch(`${base}/api?route=users%2Frole`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({userId:member.id,role:'viewer'})});
  assert.equal(failedRole.status,500);
  assert.equal((await api('users/list')).users.find(user=>user.id===member.id).role,'requester');
  ...
  const viewerCancel=await fetch(`${base}/api?route=tasks%2Fcancel`,{method:'POST',headers:{cookie:requesterCookie,'content-type':'application/json'},body:JSON.stringify({taskId:gated.task.id})});
  ...
  const stoppedSession=await fetch(`${base}/api?route=auth%2Fme`,{headers:{cookie:requesterCookie}});
  assert.equal(stoppedSession.status,401);
  const disabledLogin=await fetch(`${base}/api?route=auth%2Flogin`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({username:'requester',password:'requester-password-123'})});
  ...
  const previousDigest=auditDb.prepare("SELECT digest FROM users WHERE username='owner'").get().digest;
  auditDb.exec("CREATE TRIGGER fail_password_event BEFORE INSERT ON events WHEN NEW.type='password_changed' BEGIN SELECT RAISE(ABORT,'password event failure'); END;");
  const failedPassword=await fetch(`${base}/api?route=auth%2Fpassword`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({currentPassword:'smoke-test-password-123',newPassword:'new-smoke-password-123'})});
  assert.equal(failedPassword.status,500);
  assert.equal(auditDb.prepare("SELECT digest FROM users WHERE username='owner'").get().digest,previousDigest);
  assert.equal((await api('auth/me')).user.username,'owner');
  auditDb.exec('DROP TRIGGER fail_password_event');
  auditDb.close();
  ...
  const outdatedReassign=await fetch(`${base}/api?route=tasks%2Freassign`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:strandedStep.id,deviceId:paired.device.id})});
  ...
  const unableReassign=await fetch(`${base}/api?route=tasks%2Freassign`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:strandedStep.id,deviceId:paired.device.id})});
  ...
  const lateReassign=await fetch(`${base}/api?route=tasks%2Freassign`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:reassigned.id,deviceId:paired.device.id})});
  ...
  const duplicateRetry=await fetch(`${base}/api?route=tasks%2Fretry-plan`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:retryable.id})});
  ...
  const late=await api('connector/result',{taskId:uncertainStep.id,leaseId:uncertainStep.lease_id,success:true,result:'通信断後に端末から届いた成果'},paired.token);
  assert.equal(late.needsReview,true);
  assert.equal((await api('connector/result',{taskId:uncertainStep.id,leaseId:uncertainStep.lease_id,success:true,result:'通信断後に端末から届いた成果'},paired.token)).alreadyRecorded,true);
  const lateDetail=await api(`tasks/detail/${uncertain.id}`);
  assert.equal(lateDetail.task.status,'needs_review');
  assert.equal(lateDetail.children.find(step=>step.id===uncertainStep.id).result,'通信断後に端末から届いた成果');
  assert.equal(lateDetail.events.filter(item=>item.type==='late_result').length,1);
  ...
  const secondReconcile=await fetch(`${base}/api?route=tasks%2Freconcile`,{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({taskId:uncertainStep.id,resolution:'completed',note:'重複'})});
  assert.equal(secondReconcile.status,409);
  const earlyReview=(await api('command',{text:'結果が届く前に人が確認する仕事'})).task;
  const earlyPlan=(await api('connector/claim',{},paired.token)).job;
  await api('connector/result',{taskId:earlyPlan.id,leaseId:earlyPlan.lease_id,success:true,result:JSON.stringify({steps:[{prompt:'遅れて結果を返す',deviceId:paired.device.id}]})},paired.token);
  const earlyStep=(await api('connector/claim',{},paired.token)).job;
  const earlyDb=new DatabaseSync(path.join(data,'rei.sqlite'));
  earlyDb.prepare('UPDATE tasks SET lease_until=? WHERE id=?').run(Date.now()-1,earlyStep.id);
  earlyDb.close();
  await api('bootstrap');
  await api('tasks/reconcile',{taskId:earlyStep.id,resolution:'completed',note:'端末の成果物を先に確認した'});
  assert.equal((await api(`tasks/detail/${earlyReview.id}`)).task.status,'completed');
  const afterEarlyReview=await api('connector/result',{taskId:earlyStep.id,leaseId:earlyStep.lease_id,success:true,result:'遅れて届いた実行結果'},paired.token);
  assert.equal(afterEarlyReview.needsReview,true);
  const earlyDetail=await api(`tasks/detail/${earlyReview.id}`);
  assert.equal(earlyDetail.task.status,'needs_review');
  assert.equal(earlyDetail.children.find(step=>step.id===earlyStep.id).lateReport,'遅れて届いた実行結果');
  assert.equal(earlyDetail.children.find(step=>step.id===earlyStep.id).result,'端末の成果物を先に確認した');
  assert.equal((await api('connector/result',{taskId:earlyStep.id,leaseId:earlyStep.lease_id,success:true,result:'遅れて届いた実行結果'},paired.token)).alreadyRecorded,true);
  await api('tasks/reconcile',{taskId:earlyStep.id,resolution:'completed',note:'遅れて届いた報告も照合した'});
  assert.equal((await api(`tasks/detail/${earlyReview.id}`)).task.status,'completed');
