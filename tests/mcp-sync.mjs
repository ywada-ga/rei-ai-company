...
const statePath=path.join(dir,'mcp-sync.json');
writeFileSync(`${statePath}.tmp`,'{"rei_abcdef123456":{"url":"https://example.com/mcp"}}');
assert.throws(()=>syncMcp(configPath,[],command),/保存途中ファイル/);
assert.equal(entries[integration.name].url,'https://another.example/mcp');
unlinkSync(`${statePath}.tmp`);
writeFileSync(statePath,'{invalid');
assert.throws(()=>syncMcp(configPath,[],command),/読み取れません/);
assert.equal(entries[integration.name].url,'https://another.example/mcp');
