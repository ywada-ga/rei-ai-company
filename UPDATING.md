...
データの復元が必要な場合は、`node backup.mjs verify <バックアップフォルダ>` で確認し、`node backup.mjs restore <バックアップフォルダ> <新しい空のフォルダ>` で別の場所に展開します。稼働中の `data/` を直接上書きしません。復元先で内容を確認してから、Hubの `REI_DATA_DIR` にそのフォルダを指定します。
