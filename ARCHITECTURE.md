...
SQLiteはWALモード。プロジェクト、仕事、子仕事、端末、利用者、イベントを持つ。データ形式の版をSQLiteに記録し、現在のREIより新しい形式なら変更せず起動を止める。プロジェクトの目的は計画担当と実行担当のOpenClawへ渡す。ブラウザにはHttpOnly・SameSite=StrictのセッションCookieを使う。所有者/管理者はプロジェクトを作成・変更し、仕事を直接依頼できる。依頼者の仕事は承認後に実行し、閲覧者は参照のみ。HubのREST APIが権限を検査する。
...
バックアップ先は既定の `data/backups/` に加え、Hubの `REI_BACKUP_DIR` で絶対パスを指定できる。手動・日次バックアップはHubの保存先を使い、自動起動設定にも指定を保存する。中心PCの診断を手動実行するときは同じ環境変数を渡す。外部保存先が使えないときは既定の場所へ自動で切り替えない。
