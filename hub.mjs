...
async function api(req,res,route) {
  ...
  if(route==='chatwork/configure'&&req.method==='POST') {if(user.role!=='owner')return error(res,403,'所有者だけが設定できます');const data=await body(req);return send(res,200,configureChatwork(db,root,text(data.roomId,30),text(data.token,300)));}
