...
Hubと各PCのREI版は「司令室」と「端末・設定」に表示されます。版が異なるPCは更新が必要と表示されます。PCを更新するときは、進行中の仕事を確認してから [更新と復旧の手順](UPDATING.md) に従ってください。古いConnectorで版が送られない場合は「版未報告」と表示されます。
...
接続時にはChatworkの[自分のアカウントID](https://developer.chatwork.com/reference/get-me)も確認します。REI自身の投稿と投稿者を確認できないメッセージは回答として扱いません。投稿IDを確認できない送信は二重投稿せず「要確認」にします。
...
実行中の端末を解除すると、その端末のトークンは直ちに失効します。担当していた計画は別の計画可能なPCへ戻り、実行中の工程は「要確認」になります。解除したPCに割り当て済みの未着手工程は、ミッション詳細から接続中のPCへ移してください。
...
通信断の後に元の端末から結果が届いた場合は、工程の詳細にその報告を表示します。報告だけでは実施済みと断定せず、所有者か管理者が確認結果を記録します。

ミッション一覧は最新100件から表示し、古い仕事は **過去の仕事をさらに表示** で順に読み込めます。仕事の詳細では省略していない結果と工程履歴を確認できます。
...
- Connectorの結果報告がHubに受理されない場合、結果は参加PCの `data/pending-results.json` に残り、そのPCは次の仕事を取得しません。司令室と「端末・設定」に送信待ち件数を表示します。まずHub上の仕事の状態と参加PCのConnectorログを確認してください。ファイルには実行結果が含まれるため、照合前に削除しないでください。
- 送信待ち結果のファイルが壊れている、または `pending-results.json.tmp` が残っている場合、Connectorはそのファイルを保持して起動を止めます。仕事の実施状況とファイル内容を照合してから復旧してください。
- MCP設定の記録が壊れている、または `mcp-sync.json.tmp` が残っている場合、MCPの追加・解除を止めて記録を保持します。OpenClawの現設定と照合してから復旧してください。
- 計画だけが失敗した仕事は、所有者・管理者がミッション詳細から再実行できます。実行結果が不明な工程は、端末で確認した結果を記録してください。
- 計画結果の形式が崩れた場合も、元の指示を自動実行せず「要確認」にします。
- 停止中のPCに割り当てられた未着手の実行工程は、所有者・管理者がミッション詳細から接続中のPCへ移せます。実行開始後は移せません。

## バックアップと復元

所有者は **端末・設定 → バックアップを作成** から、REIの稼働中にSQLiteの整合した写しを作れます。保存先は中心PCの `data/backups/` です。仕事・利用者・端末・Chatwork暗号鍵・このPCのConnector設定と、送信待ちの結果を含みます。MCP設定と送信待ち結果の保存途中ファイルが残っている場合も保全します。対象ファイルがシンボリックリンクなど通常のファイルではない場合は、欠けたバックアップを作らずエラーにします。バックアップには認証情報が入るため、フォルダの権限は所有者だけに制限します。別の故障にも備えるには、作成後のフォルダを外部ストレージへコピーしてください。`owner-credentials.txt` は含みません。
...
復元は既存データを上書きせず、必ず新しいフォルダに展開します。検証ではファイルのSHA-256とSQLiteの整合性を確認します。復元後はHubとConnectorを停止し、両方で `REI_DATA_DIR` を復元先に指定して起動してください。自動起動を使う場合は、復元先を指定した状態で各OSのインストーラーを再実行すると、次回のログイン後も同じ保存先を使います。端末トークンなどの秘密情報を含むため、バックアップを公開リポジトリに追加しないでください。

GitHubから更新するときは、実行中の仕事の確認・バックアップ・検査・OS別の再起動をまとめた [更新と復旧の手順](UPDATING.md) を参照してください。
