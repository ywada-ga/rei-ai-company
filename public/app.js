...
function render() {
  ...
  $('system-signals').innerHTML = `<div class="signal-row"><span>REI HUB</span><b class="online">● ONLINE</b></div><div class="signal-row"><span>OPENCLAW端末</span><b class="${live ? 'online' : 'offline'}">${workers.filter(w=>w.connected).length} / ${workers.length} CONNECTED</b></div><div class="signal-row"><span>計画担当</span><b>${primaryPlanner?'READY':standbyPlanner?'予備PCが引継ぎ可能':'WAITING'}</b></div>${pendingResults?`<div class="signal-row"><span>結果の送信待ち</span><b class="offline">${pendingResults}件 · 端末を確認</b></div>`:''}<div class="signal-row"><span>人の回答待ち</span><button id="human-pending-open" ${state.data.humanPending.length?'':'disabled'}>${state.data.humanPending.length}件 ↗</button></div><div class="signal-foot">端末の登録と状態は「端末・設定」で確認できます</div>`;
...
async function refreshSettings() {
  ...
  try {
    ...
    $('device-management').innerHTML = devices.devices.length ? devices.devices.map(d=>`<div class="setting-device"><span><b>${escapeHtml(d.label)}</b><small>${d.online?'● 接続中':'○ 未接続'}${d.planner?' · REI計画担当':''}${d.agent_name?` · OpenClaw: ${escapeHtml(d.agent_name)}`:''}${d.pending_results?` · 結果送信待ち ${d.pending_results}件。端末の接続と仕事の状態を確認してください`:''}</small></span><button data-revoke="${escapeHtml(d.id)}" class="outline-button">解除</button></div>`).join('') : '<p>端末はまだ登録されていません。</p>';
