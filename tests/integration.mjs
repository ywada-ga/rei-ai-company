...
try {
  ...
  const failedProjectStatus=await fetch('http://127.0.0.1:4181/api?route=projects%2Fstatus',{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({projectId:project.id,status:'paused'})});
  ...
  const failedRole=await fetch('http://127.0.0.1:4181/api?route=users%2Frole',{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({userId:member.id,role:'viewer'})});
  ...
  const failedPassword=await fetch('http://127.0.0.1:4181/api?route=auth%2Fpassword',{method:'POST',headers:{cookie,'content-type':'application/json'},body:JSON.stringify({currentPassword:'smoke-test-password-123',newPassword:'new-smoke-password-123'})});
