...
if(process.platform==='darwin') {
  ...
  for(const mode of ['hub','connector']) {
    const result=spawnSync(process.execPath,['install-macos.mjs',mode],{cwd:root,encoding:'utf8',env:{...process.env,PATH:`${bin}${path.delimiter}${process.env.PATH}`,REI_LAUNCH_AGENTS_DIR:agents,REI_DATA_DIR:data,REI_BACKUP_DIR:path.join(temp,'separate-backups'),REI_CONNECTOR_CONFIG:path.join(data,'connector.json'),REI_PORT:'4188',REI_JOB_TIMEOUT_SECONDS:'3600'}});
    ...
    assert.match(plist,/<key>REI_BACKUP_DIR<\/key><string>.*separate-backups<\/string>/);
